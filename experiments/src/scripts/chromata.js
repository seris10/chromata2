...
import PreprocessingPipeline from './preprocessing';
...
export default class Chromata {
    ...
    constructor(imageElement, options = {}) {
        ...
        // Initialize face detection and preprocessing
        this.faceDetection = new FaceDetection();
        this.preprocessing = new PreprocessingPipeline(renderCanvas);
        this.detectedFaces = [];
        this.faceMask = null;
        this.edgeMap = null;
        ...
        image.addEventListener('load', async () => {
            ...
            if (this.options.preprocessEdges) {
                ...
                if (this.preprocessing.enabled) {
                    ...
                    this.edgeMap = this.preprocessing.createEdgeMap(imageData);
                }
            }

            // Detect faces if enabled
            if (this.options.detectFaces) {
                const faceDetectionReady = await this.faceDetection.initialize();
                if (faceDetectionReady) {
                    this.detectedFaces = this.faceDetection.detectFaces(sourceCanvas);
                    this.faceMask = this.faceDetection.createFaceMask(
                        dimensions.width,
                        dimensions.height,
                        this.detectedFaces,
                        0.1 // 10% padding
                    );
                    console.log(`Detected ${this.detectedFaces.length} face(s)`);
                }
            }
